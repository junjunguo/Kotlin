...
export class AuthInterceptor implements HttpInterceptor {
  ...
  public intercept(
    ...
    if (
      this.auth &&
      this.auth.getAuthModel() &&
      this.auth.getAuthModel() &&
      this.auth.getAuthModel().access_token
    ) {
    ...
    return next.handle(authReq).catch(err => {
      ...
      if (
        ...
        err.status === 401 &&
        this.isRefreshTokenExpired(err)
      ) {
        this.auth.logout();
        return Observable.throw(err);
      } else if (
  ...
  private tryRenewAccessToken(
    req: HttpRequest<any>,
    next: HttpHandler,
    err: HttpErrorResponse
  ) {
    this.auth.accessTokenExpired();
    return this.auth.renewAccessToken().switchMap(res => {
      if (res) {
        return next.handle(this.appendToken(res.access_token, req));
      } else {
        return Observable.throw(err);
      }
    });
  ...
  private isRefreshTokenExpired(err: HttpErrorResponse) {
    return (
      ...
      err.error &&
      err.error.error_description &&
      err.error.error_description.includes("Invalid refresh token (expired)")
    );
  }
  ...
  private isInvlidTokens(err: HttpErrorResponse) {
    return (
      ...
      err.error &&
      err.error.error_description &&
      err.error.error_description.includes(
